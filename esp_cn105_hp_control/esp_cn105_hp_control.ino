...
void setup() {
  ...
  lastTempSend = millis();
  blink(100, 100); blink(100, 100); blink(100, 100);
  digitalWrite(blueLedPin, LOW);
  mqtt_client.publish(heatpump_debug_topic, "Hello world");
}

void blink(int time_on, int time_off) {
  digitalWrite(blueLedPin, LOW); delay(time_on);
  digitalWrite(blueLedPin, HIGH); delay(time_off);
  digitalWrite(blueLedPin, LOW); delay(time_on);
  digitalWrite(blueLedPin, HIGH); delay(time_off);
  digitalWrite(blueLedPin, LOW); delay(time_on);
  digitalWrite(blueLedPin, HIGH); delay(time_off);
